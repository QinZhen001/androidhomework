package com.example.qinzhen.androidhomework.tab;
...
public class BottomTabBar extends LinearLayout {
    ...
    private void init(final List<BarEntity> bars) {
        ...
        ViewGroup.LayoutParams llp = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,2);    //这里设计分割线
        addView(line,llp);
        /**
         * 添加底部导航栏菜单
         */
    ...
    public void select(int position, List<BarEntity> bars) {
        ...
        for (int i = 0; i < bottom.getChildCount(); i++) {
            ...
            if (position == i) {
                ...
                LayoutParams para = new LayoutParams((int) getResources().getDimension(R.dimen.image_select_height), (int) getResources().getDimension(R.dimen.image_select_height));
                image.setLayoutParams(para);
                text.setTextColor(textSelectColor);
                text.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
            } else {
                image.setImageResource(bars.get(i).getTabUnSelectedResId());
                LayoutParams para = new LayoutParams((int) getResources().getDimension(R.dimen.image_height), (int) getResources().getDimension(R.dimen.image_height));
                image.setLayoutParams(para);
                text.setTextColor(textColor);
                text.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
            }
